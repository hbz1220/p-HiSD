# Section 7.2.1: compare attraction trajectories on the butterfly potential.
# Save the contour and trajectory plot as Figure 2: outputs/7.2/7.2.1/2.1.pdf.
...
def run_hisd_2d(x0, grad_fn, hess_fn, k, dt_x, dt_v, tol, max_iter,
                precond_fn=None, precond_kwargs=None, bound=5.0, v_iter=5):
    """Run HiSD on a 2D problem with trajectory recording and overflow protection."""
    from scipy.linalg import eigh
    x = np.array(x0, dtype=float)
    H = hess_fn(x)

    # Initialize the unstable direction from H v = lambda M v (M = I for HiSD).
    ...
    for it in range(max_iter):
        ...
        # Stop at the gradient tolerance or when the trajectory becomes unsafe.
        ...
        # Refresh the metric at the new state before the frame sweeps.
...
def run():
    ...
    # Use the two prescribed starts to compare the attraction trajectories.
    ...
    # Spectral metric: M(x) = |H(x)| + eps_spec I.
    eps_spec = 1e-2
    ...
    # Retain directional history when updating the subspace-inertial metric.
    ...
    for it in range(max_iter):
        ...
        try:
            for _ in range(5):  # inner iterations for v
                Hv = pc['solve'](H @ v)
                M_mat = pc['mat']
                proj = Hv - v * (v @ (M_mat @ Hv))
                v = v - dt_v * proj
    ...
    # Locate stationary points separately for the plot markers; require index 1.
    # This search does not alter the computed trajectories.
    ...
    plt.close(fig)
    print(f"Figure 2.1 saved to {out_dir}/2.1.pdf")
