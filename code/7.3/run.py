...
def M_orthonormalize(V, precon):
    ...
    for i in range(k):
        ...
        Q[:, i] = vi / nrm
    return Q
def dense_M_from_precon(precon, n):
    E = np.eye(n)
    M = np.column_stack([precon.apply_M(E[:, j]) for j in range(n)])
    return 0.5 * (M + M.T)
def generalized_initial_frame(H_ref, precon, k, name):
    M_dense = dense_M_from_precon(precon, H_ref.shape[0])
    _, V = la.eigh(H_ref, M_dense, subset_by_index=[0, k - 1])
    V = M_orthonormalize(V, precon)
    if V is None:
        raise RuntimeError(f"{name}: generalized initial M-orthonormalization failed.")
    G = np.array([[precon.M_inner(V[:, i], V[:, j]) for j in range(k)] for i in range(k)])
    if not np.allclose(G, np.eye(k), atol=1e-8, rtol=1e-8):
        raise RuntimeError(f"{name}: initial frame is not M-orthonormal.")
    return V
def _finite(*arrs):
...
def main():
    ...
    V0_jac = generalized_initial_frame(H_ref, precon_jac, cfg.k, "Jacobi")
    V0_blk = generalized_initial_frame(H_ref, precon_blk, cfg.k, "Block-Jacobi")
