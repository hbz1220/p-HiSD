"""
Plot MATLAB experiment results.
Reads experiment_results.mat and uses the same plotting style as run.py.
"""
...
def plot_matlab_results():
    # Read the .mat file
    mat_path = 'experiment_results.mat'
    ...
    # Extract gradient norms (Note: MATLAB struct array access pattern)
    # The shape of grad_norm is (1, n_iters), which needs to be flattened to (n_iters,)
    ...
    # --- Plotting (using the same style as run.py) ---
    ...
    for name, gn in results.items():
        ...
        # Removed marker, markersize, and markevery parameters
    ...
    output_dir = '../../figures'
    os.makedirs(output_dir, exist_ok=True)  # Ensure the directory exists; create it if it doesn't
