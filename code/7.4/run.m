...
hist_standard = run_hisd(X0, N, K, dt_x_standard, dt_v_standard, v_iter, 300, 1e-6, 'standard');
...
function history = run_hisd(X0, N, K, dt_x, dt_v, v_iter, max_iter, tol, method)
    ...
    elseif strcmp(method, 'frozen_spectral')
        ...
        % 构造论文中的 Frozen Spectral metric 权重
        eps_M = 1e-2;
        lambda_metric = abs(lambda_vals) + eps_M;

        % M^{-1} = U diag(lambda_metric^{-1}) U^T
        ...
        fprintf('  最大特征值: %.6f\n', max(lambda_vals));
        fprintf('  eps_M: %.2e\n', eps_M);
        fprintf('  metric 最小权重: %.6f\n', min(lambda_metric));
        fprintf('  metric 最大权重: %.6f\n', max(lambda_metric));
    ...
    for iter = 1:max_iter
        ...
        elseif strcmp(method, 'ic')
            ...
            % 在 X_{m+1} 上重建 M 用于 frame-update，并保留给后续 reinitialize
            [L, M, ~] = build_shifted_ic_preconditioner(X, N, K);
            history.precond_constructions = history.precond_constructions + 1;
...
function V = initialize_frame(X, N, K, method, L, M, U_spectral, Lambda_inv)
    ...
    elseif strcmp(method, 'ic')
        ...
        % M-归一化: v^T M v = 1
        % 1. Project v to eigenspace
        u_proj = U_spectral' * v;

        % 2. Compute v^T M v = u^T Lambda u
...
function V_new = update_frame_frozen_spectral(X, V, N, K, dt_v, v_iter, U_spectral, Lambda_inv)
    ...
    for iter_v = 1:v_iter
        ...
        % M-归一化
        % 1. From Lambda_inv restore metric weights
        lambda_metric = 1.0 ./ diag(Lambda_inv);
