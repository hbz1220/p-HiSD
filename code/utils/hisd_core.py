...
def _update_v_iterative(V, H, dt_v, k, M=None, v_iter=1):
    ...
    for _ in range(v_iter):
        for i in range(k):
            ...
            # Remove the current Rayleigh component and reflect lower-index directions.
            proj = Hv.copy()
            if M is not None:
            ...
            # Restore orthogonality to the previously updated directions.
            if M is not None:
                Mmat = M['mat']
                for j in range(i):
...
def hisd_solve(x0, grad_E, hess_E, k=1,
    ...
    for m in range(max_iter):
        ...
        # This generic solver stops on the gradient norm; index certification is the caller's task.
        if gn < tol:
            converged = True
            break

        # Reflect the gradient along the k unstable directions.
        if precond is not None:
        ...
        # Update the frame at the new state.
        H = hess_E(x)
        if use_eig:
