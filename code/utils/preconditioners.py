...
def subspace_inertial_precond(H, k, alpha=0.7, a_weights=None, eps=1e-2,
    ...
    if a_weights is None:
        a_weights = np.ones(k) * 0.49

    # Align eigenvector signs before blending with the previous unstable basis.
    ...
    # Weight unstable directions individually and use one weight on their complement.
    ...
    # Reuse a dense Cholesky factor; fall back to a direct inverse if factorization fails.
    M_sym = 0.5 * (M + M.T)
    try:
